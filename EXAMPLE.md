...
- **Dhriti 3 — "Refuse the cheap rescue."** *No silenced test, no ignore-pragma, no "for now"
  hack that trades the goal for a quiet gate.*
  **Falsifier:** *a suppression or skip added to make a red gate look green.*
- **Viveka 3 — "Verify the confident answer."** *The claim you did not just check is the one
  to check; confidence is not evidence.* → look at what the change did to rounding, not at the
  test.
